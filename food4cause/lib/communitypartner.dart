...
class CommunityPartner extends StatelessWidget {
  ...
  makeMarker(latlocation, Location location) {
    return Marker(
        point: latlocation,
            ...
            onPressed: () {
              showModalBottomSheet(
                  ...
                  builder: (BuildContext context) {
                    _mapController.move(location.latlng, 16);
  ...
  Widget build(BuildContext context) {
    for (int i = 0; i < locations.length; i++) {
      markerList.add(makeMarker(locations[i].latlng, locations[i]));
    }
    return Scaffold(
      appBar: AppBar(
      ...
      body: FlutterMap(
        ...
        options: MapOptions(zoom: 10, center: positionCoords),
        layers: [
          TileLayerOptions(
              urlTemplate:
